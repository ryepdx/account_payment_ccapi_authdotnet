...
class auth_net_cc_api(osv.osv):
    ...
    def validate_sales_receipt(self, cr, uid, ids, context={}):
        ...
        for voucher_obj in voucher_objs:
            ...
            sale_receipts_ids = voucher_obj.search(cr, uid, [('type', '=', 'sale'), ('rel_sale_order_id', '=', rel_sale_order_id), ('state', 'in', ['draft'])])
            
            if not sale_receipts_ids:
                sale_receipts_id = self.pool.get('sale.order').create_sales_receipt(cr, uid, [rel_sale_order_id], context=context)
                sale_receipts_ids = [sale_receipts_id]
            voucher_obj.action_move_line_create(cr, uid, sale_receipts_ids, context=context)
        return True
    ...
    def do_this_transaction(self, cr, uid, ids, refund=False, context=None):
        ...
        if acc_voucher_obj.cc_save_card_details:
            if not acc_voucher_obj.cc_bank:
                raise osv.except_osv(_('No Bank selected!'),
                                     _("Please select the bank to save credit card details on customer bank details."))
            elif not partner_bank_obj.search(cr, uid, [('cc_number', '=', creditcard),
                ...
                if acc_voucher_obj.cc_state:
                    state_ids = self.pool.get('res.country.state').search(cr, uid,
                                                                          [('name', '=', acc_voucher_obj.cc_state)])
                    state_id = state_ids and state_ids[0] or False

                if acc_voucher_obj.cc_country:
                    country_ids = self.pool.get('res.country').search(cr, uid,
                                                                      [('name', '=', acc_voucher_obj.cc_country)])
                ...
                partner_bank_obj.create(cr, uid, {
                    ...
                    'street': str(acc_voucher_obj.cc_b_addr_1) + '-' + str(
                        acc_voucher_obj.cc_b_addr_2),
        ...
        if refund:
            ...
            if not acc_voucher_obj.journal_id.cc_allow_refunds:
                raise osv.except_osv(_('Unable to do Refund!'),
                                     _("Please check \"Allow Credit Card Refunds\" on journal to do refund."))
            ...
            if not acc_voucher_obj.cc_trans_id:
                raise osv.except_osv(_('No Transaction ID!'),
                                     _("Cannot proceed to Charge, please pre-authorize the transaction first!!"))

        else:
            raise osv.except_osv(_('No "Type of transaction"!'),
                                 _("No Transaction type selected. Please select pre-authorize or charge"
                                   " from \"Type of transaction\" section."))
        ...
        parameters = acc_voucher_obj.cc_name and voucher_obj.setParameter(
            parameters, 'x_first_name', acc_voucher_obj.cc_name or None) or parameters
        parameters = acc_voucher_obj.cc_b_addr_1 and voucher_obj.setParameter(
            parameters, 'x_address', acc_voucher_obj.cc_b_addr_1 or None) or parameters
        parameters = acc_voucher_obj.cc_city and voucher_obj.setParameter(
            parameters, 'x_city', acc_voucher_obj.cc_city or None) or parameters
        parameters = acc_voucher_obj.cc_state and voucher_obj.setParameter(
            parameters, 'x_state', acc_voucher_obj.cc_state or None) or parameters
        parameters = acc_voucher_obj.cc_zip and voucher_obj.setParameter(
            parameters, 'x_zip', acc_voucher_obj.cc_zip or None) or parameters
        parameters = acc_voucher_obj.cc_country and voucher_obj.setParameter(
            parameters, 'x_country', acc_voucher_obj.cc_country or None) or parameters
        ...
        if x_type == 'AUTH_ONLY':
            ...
            voucher_obj._historise(cr, uid, acc_voucher_obj.id, 'Authorization', trans_id=results[6], status=status,
                                   amount=total)
            ...
            voucher_obj.write(cr, uid, ids, ret_dic)
            voucher_obj._historise(cr, uid, acc_voucher_obj.id, 'Capture', trans_id=results[6], status=status,
            ...
            if results[0] == '1':
                ...
                '''
                # self.validate_sales_receipt(cr, uid, ids, context=context)
                '''
            ...
            ret_dic['cc_status'] = status
            voucher_obj._historise(cr, uid, acc_voucher_obj.id, 'Refund', trans_id=results[6], status=status,
                                   amount=acc_voucher_obj.cc_refund_amt)
            if results[0] == '1':
                ret_dic['cc_transaction'] = False
                refund_journal_id = False
                j_ids = self.pool.get('account.journal').search(
                ...
                if refund_journal_id and context.get('cc_refund'):
                    ...
                    inv_vals.update(
                        invoice_obj.onchange_partner_id(cr, uid, [], 'out_refund', acc_voucher_obj.partner_id.id, '',
                                                        '', '', '')['value'])
                    inv_lines = []
                    ...
                    for line in refund_lines:
                        ...
                        onchange_vals = \
                        invoice_line_obj.product_id_change(cr, uid, [], line['product_id'], False, line['qty'], '',
                                                           'out_refund', inv_vals['partner_id'], context={})['value']
                        onchange_vals['price_unit'] = line['price_unit']
                        inv_line_vals.update(onchange_vals)
